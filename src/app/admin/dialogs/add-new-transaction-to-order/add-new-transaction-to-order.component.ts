...
export class AddNewTransactionToOrderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.data.mode==='edit-transaction'){
      ...
      this.requiredDate=new Date(this.data.item.requiredDate);
      this.quantity=this.data.item.transactionQuantity;
      this.expirationDate=new Date(this.data.item.expirationDate);
  ...
  saveTransaction() {
    ...
    if(this.data.mode==='add-trans'){
      ...
      payload['itemNum']=this.data.itemNumber
    }else{
      payload['itemNum']=this.data.item.itemNumber
      payload['id']=this.data.item.id
    }
    this.transactionService
      .get(payload, `/Admin/${this.data && this.data.mode==='add-trans'? 'TransactionForOrderInsert':'TransactionForOrderUpdate' }`)
      .subscribe(
        (res: any) => {
          if (res.isExecuted) {
